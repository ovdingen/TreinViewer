...
class DVS {
    ...
    function getJourney(string $serviceCode) {
        $stmt = $this->db->prepare("SELECT * FROM " . $this->tableName . " WHERE treinnummer = :servicecode ORDER BY receive DESC LIMIT 1"); // table and column names cannot be replaced by parameters in PDO
