...
function verwerkNSDVSBericht(string $contents, PDO $db) {
    ...
    foreach($xml->ReisInformatieProductDVS->DynamischeVertrekStaat as $data) {
        $stmt = $db->prepare("INSERT INTO dvs VALUES(:receive, :transportmodecode, :treinnummer, :treinformule, :materieel, :vervoerder, :treineindbestemming, :toeslag, :reserveren, :nietinstappen, :speciaalkaartje)");
        $db_fields = ["receive" => time(), "transportmodecode" => (string) $data->Trein->TreinSoort , "treinnummer" => (string) $data->RitId, "treinformule" => (int) $data->Trein->TreinFormule, "materieel" => json_encode($data->Trein->TreinVleugel->MaterieelDeelDVS), "vervoerder" => (string) $data->Trein->Vervoerder, "treineindbestemming" => json_encode($data->Trein->TreinVleugel->TreinVleugelEindBestemming), "toeslag" => (string) $data->Trein->Toeslag, "reserveren" => (string) $data->Trein->Reserveren, "nietinstappen" => (string) $data->Trein->NietInstappen, "speciaalkaartje" => (string) $data->Trein->SpeciaalKaartje];
