...
function verwerkNSDVSBericht(string $contents, PDO $db) {
    ...
    foreach($xml->ReisInformatieProductDVS->DynamischeVertrekStaat as $data) {
        $stmt = $db->prepare("UPDATE dvs VALUES(:receive, :transportmodecode, :treinnummer, :treinformule, :materieel, :vervoerder, :treineindbestemming, :toeslag, :reserveren, :nietinstappen, :speciaalkaartje, :stopstations)");
